...
const ALLOWED_BASE_URLS = new Set([DEFAULT_PUBLIC_BASE_URL, DEFAULT_PRO_BASE_URL]);
const ALLOWED_API_ENVIRONMENTS = new Set(['auto', 'public', 'pro']);
...
const resolveBaseUrl = (authData = {}) => {
  ...
  const apiEnvironment = (authData.apiEnvironment || 'auto').trim().toLowerCase();
  const apiKey = (authData.apiKey || '').trim();

  if (baseUrl && ALLOWED_BASE_URLS.has(baseUrl)) {
    return baseUrl;
  }

  if (ALLOWED_API_ENVIRONMENTS.has(apiEnvironment)) {
